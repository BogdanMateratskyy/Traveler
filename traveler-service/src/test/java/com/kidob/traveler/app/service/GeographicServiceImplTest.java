...
public class GeographicServiceImplTest {
	...
	private static final int DEFAULT_SITY_ID = 1;
	...
	public void testFindCityByIdSuccess() {
		...
		Optional<City> foundCity = service.findCityById(DEFAULT_SITY_ID);
		assertTrue(foundCity.isPresent());
		assertEquals(foundCity.get().getId(), DEFAULT_SITY_ID);
	}
	...
	public void testSearchStationsByTransportTypeSuccess() {
		...
		List<Station> stations = service.searchStations(
				new StationCriteria(TransportType.AUTO), new RangeCriteria(1, 5));
		assertNotNull(stations);
		assertEquals(stations.size(), 2);
	}
	...
	public void testSearchStationsByTransportTypeNotFound() {
		...
		List<Station> stations = service.searchStations(
				new StationCriteria(TransportType.AVIA), new RangeCriteria(1, 5));
		assertNotNull(stations);
		assertTrue(stations.isEmpty());
	}
}
